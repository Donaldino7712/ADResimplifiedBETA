...
export class DimBoost {
  ...
  static get maxDimensionsUnlockable() {
    return NormalChallenge(7).isRunning ? 6 : 8;
  }
  ...
  static get maxBoosts() {
    ...
    if (InfinityChallenge(1).isRunning) {
      // Usually, in Challenge 6, the only boosts that are useful are the first 5
      // (the fifth unlocks sacrifice). In IC1 (Challenge 6 and Challenge 7
  ...
  static get lockText() {
    if (DimBoost.purchasedBoosts >= this.maxBoosts) {
      ...
      if (NormalChallenge(6).isRunning) return "Locked (6th Antimatter Challenge)";
    }
    return null;
  }
  ...
  static bulkRequirement(bulk) {
    ...
    const discount = Effects.sum(
      InfinityChallenge(5).effects.dimBoost,
      TimeStudy(211),
      TimeStudy(222)
    );
    if (tier === 6 && NormalChallenge(7).isRunning) {
  ...
  static get unlockedByBoost() {
    ...
    } else if (boosts === 4 && !NormalChallenge(7).isRunning && !EternityChallenge(3).isRunning) {
  ...
  static get startingDimensionBoosts() {
    if (InfinityUpgrade.skipResetSacrifice.isBought) return 5;
