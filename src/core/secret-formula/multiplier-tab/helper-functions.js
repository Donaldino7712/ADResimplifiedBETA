...
export const MultiplierTabHelper = {
  ...
  decomposeTickspeed() {
    ...
    if (effectiveCount < 3) {
      ...
      const baseMult = 0.965 ** 2 / (NormalChallenge(3).isRunning ? 0.83 : 0.8);
      const logBase = Math.log10(baseMult);
  ...
  // Helper method to check for whether an NC reward affects a particular dimension or not, see achievementDimCheck()
  NCDimCheck(nc, dimStr) {
    switch (nc) {
      case 1:
