...
export const AD = {
  total: {
    ...
    displayOverride: dim => {
      if (dim) {
        const singleMult = NormalChallenge(8).isRunning
      ...
      if (NormalChallenge(8).isRunning) return `${format(MultiplierTabHelper.actualNC12Production(), 2)}/sec`;
  ...
  effectNC: {
    ...
    displayOverride: dim => {
      ...
      if (NormalChallenge(2).isRunning) {
        dimMults[1] = dimMults[1].times(player.chall2Pow);
      }
    ...
    isActive: () => [2, 8].some(c => NormalChallenge(c).isRunning),
    icon: MultiplierTabIcons.CHALLENGE("infinity"),
  },
  nerfIC: {
