...
export function getDimensionFinalMultiplierUncached(tier) {
  ...
  if (NormalChallenge(7).isRunning && tier > 6) return DC.D1;
...
function applyNDMultipliers(mult, tier) {
  ...
  if (tier !== 8 && InfinityChallenge(4).isRunning) {
    multiplier = multiplier.times(Sacrifice.totalBoost.sqrt());
  }
...
class AntimatterDimensionState extends DimensionState {
  constructor(tier) {
    ...
    const C4_BASE_COSTS = [null, 10, 50, 100, 250, 1250, 1e4, 1e5, 2e6];
    this._c4BaseCost = C4_BASE_COSTS[tier];
    const C4_BASE_COST_MULTIPLIERS = [null, 1e3, 2e3, 3e3, 5e3, 7e3, 9e3, 1.2e4, 2e4];
    this._c4BaseCostMultiplier = C4_BASE_COST_MULTIPLIERS[tier];
  }
  ...
  get costScale() {
    return new ExponentialCostScaling({
      baseCost: NormalChallenge(4).isRunning ? this._c4BaseCost : this._baseCost,
      baseIncrease: NormalChallenge(4).isRunning ? this._c4BaseCostMultiplier : this._baseCostMultiplier,
  ...
  get rateOfChange() {
    ...
    if (tier === 8 ||
      ...
      (tier > 6 && NormalChallenge(8).isRunning)) {
      return DC.D0;
    }
  ...
  get isProducing() {
    ...
    if ((EternityChallenge(3).isRunning && tier > 4) ||
      (NormalChallenge(7).isRunning && tier > 6) ||
   ...
   */
  get currencyAmount() {
    return this.tier >= 3 && NormalChallenge(4).isRunning
  ...
  set currencyAmount(value) {
    if (this.tier >= 3 && NormalChallenge(4).isRunning) AntimatterDimension(this.tier - 2).amount = value;
  ...
  get continuumValue() {
    ...
    return this.costScale.getContinuumValue(this.currencyAmount, 10) * Laitela.continuumBoost;
  }
  ...
  get isAvailableForPurchase() {
    ...
    return this.tier < 7 || !NormalChallenge(7).isRunning;
  }
...
export const AntimatterDimensions = {
  ...
  get buyTenMultiplier() {
    if (NormalChallenge(5).isRunning) return DC.D2.min(1 + DimBoost.totalBoosts / 5);
