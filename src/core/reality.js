...
export const GlyphSelection = {
  ...
  get choiceCount() {
    return GlyphGenerator.availableTypes.length;
  },
  ...
  generate(level = gainedGlyphLevel()) {
    this.glyphs = this.glyphList(level);
  },
  ...
  get upcomingGlyphs() {
    return this.glyphList(gainedGlyphLevel());
  }
};
...
export function beginProcessReality(realityProps) {
  ...
  if (glyphsToProcess < 100) {
    for (let glyphNum = 0; glyphNum < glyphsToProcess; glyphNum++) {
      GlyphSelection.generate(glyphLevel);
      if (EffarigUnlock.glyphFilter.isUnlocked) {
        const glyphChoices = GlyphSelection.glyphList(realityProps.gainedGlyphLevel);
  ...
  const glyphFunction = () => {
    if (fastToggle) {
      ...
      processAutoGlyph(realityProps.gainedGlyphLevel);
