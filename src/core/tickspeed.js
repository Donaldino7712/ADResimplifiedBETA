...
export function getTickSpeedMultiplier() {
  ...
  if (NormalChallenge(3).isRunning) baseMultiplier = 0.83;
  galaxies -= 2;
  galaxies *= effects;
...
export const Tickspeed = {
  get isUnlocked() {
    return AntimatterDimension(2).continuumAmount > 0 || EternityMilestone.unlockAllND.isReached ||
  ...
  get isAffordable() {
    return Currency.antimatter.value.gte(this.cost);
  },
  ...
  get continuumValue() {
    ...
    return this.costScale.getContinuumValue(Currency.antimatter.value, 1) * Laitela.continuumBoost;
  },
  ...
  get totalUpgrades() {
    ...
    return boughtTickspeed + player.totalTickGained + NormalChallenge(4).effectOrDefault(0);
  },
